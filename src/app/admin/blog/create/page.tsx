...
import { FileText, Save, ArrowLeft, Image as ImageIcon, Loader2 } from "lucide-react";
...
export default function CreateBlogPage() {
  ...
  }, [watchTitle, setValue, isSlugManuallyEdited]);
  ...
  const onSubmit = async (values: BlogFormValues) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message || "Gagal membuat artikel");
      }
    } catch (err: any) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="font-heading text-xl font-bold text-[var(--text-heading)]">
            Tulis Artikel Inspirasi Baru
          </h1>
          <p className="font-body text-xs text-[var(--text-muted)] mt-0.5">
            Buat konten berkualitas seputar teknik bangunan dan material untuk publik.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-xl)] p-6 md:p-8 space-y-6 shadow-sm">
          ...
          {/* Content (Rich Text Input Placeholder / Textarea) */}
          <div className="space-y-1.5">
